...
public class SliderR : MonoBehaviour
{
	...
	public void LoadElement()
	{
		...
		Global.MyCircuit.UF.Union(TL, L);
		Global.MyCircuit.UF.Union(TL, R);
		Global.MyCircuit.UF.Union(TL, TR);
	}
	public void SetElement()
	{
		...
		L = bodyItem.childsPorts[2].PortID_Global;
		R = bodyItem.childsPorts[3].PortID_Global;
		Global.MyCircuit.entities.Add(new Resistor(string.Concat(EntityID, "_L"), TL.ToString(), L.ToString(), RL));
		Global.MyCircuit.entities.Add(new Resistor(string.Concat(EntityID, "_R"), TL.ToString(), R.ToString(), RR));
		Global.MyCircuit.entities.Add(new VoltageSource(string.Concat(EntityID, "_T"), TL.ToString(), TR.ToString(), 0));
